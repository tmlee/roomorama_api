module RoomoramaApi
	class Client
		...
		def users_me(options={})
			api_call "me", options, RoomoramaApi::User
		end

		def users_update_profile(options={})
			api_call "me", options, :put, RoomoramaApi::User
		end

		def users_get_data(user_id, options={})
			api_call "users", options, RoomoramaApi::User
		end

		def users_register(options={})
			api_call "users", options, :post, RoomoramaApi::User
		end
		...
		def api_call(method_name, options, verb=:get, entity)
			options["access_token"] = @oauth_token
			response = parse_response(connection(method_name, options, verb))
			response["result"] = entity.send 'new', response["result"]
			response
		end
